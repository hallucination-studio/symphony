...
function isTargetWorkflowFacts(value) {
  return Boolean(value) && typeof value === "object" && !Array.isArray(value) &&
    Object.keys(value).every((key) => ["root", "plan", "stageExecutions", "progress", "repairEscalation", "delivery"].includes(key)) &&
    ...
    (value.repairEscalation === undefined || isRepairEscalation(value.repairEscalation));
}

function isRepairEscalation(value) {
  return value && typeof value === "object" && !Array.isArray(value) &&
    Object.keys(value).every((key) => ["findingId", "sourceVerifyId", "disposition", "breaker"].includes(key)) &&
    typeof value.findingId === "string" && typeof value.sourceVerifyId === "string" &&
    value.disposition === "escalated" && value.breaker && typeof value.breaker === "object" &&
    !Array.isArray(value.breaker) &&
    Object.keys(value.breaker).every((key) => ["checked", "decision", "cycleCount", "maxCycles", "openFindingCount"].includes(key)) &&
    value.breaker.checked === true && value.breaker.decision === "escalate" &&
    Number.isSafeInteger(value.breaker.cycleCount) && Number.isSafeInteger(value.breaker.maxCycles) &&
    Number.isSafeInteger(value.breaker.openFindingCount);
}
