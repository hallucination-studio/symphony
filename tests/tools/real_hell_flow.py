...
from performer.linear import LinearClient
...
async def _run_real_hell_flow_evidence(tmp_path: Path) -> dict[str, Any]:
    ...
    label_name = f"performer-real-flow-{run_id}"
    ...
    for flow_id in FLOW_IDS:
        issue = await client.create_issue(
            ...
            label_id=label["id"],
            title=f"{flow_id} Performer real HELL flow {run_id}",
            description=(
                f"Temporary issue for {flow_id} from Performer's real HELL flow evidence harness. "
    ...
    for flow_id in ("FLOW-006", "FLOW-007"):
        ...
        blocker = await client.create_issue(
            ...
            label_id=label["id"],
            title=f"{flow_id} blocker for Performer real HELL flow {run_id}",
    ...
    for flow_id, issue in flow_issues.items():
        await client.comment_issue(
            issue["id"],
            f"Performer real HELL flow evidence for {flow_id} completed. "
...
def _make_real_repo_fixture(tmp_path: Path) -> Path:
    ...
    subprocess.run(["git", "config", "user.name", "Performer Real Flow"], cwd=repo, check=True)
...
for raw in sys.stdin:
    ...
    if method == "initialize":
        ...
        pathlib.Path("PERFORMER_REAL_HELL_VALIDATION.md").write_text("pytest tests/test_hell_real_flow.py -q passed\\n", encoding="utf-8")
...
def _flow_bundle(
    ...
    *,
    ...
    bundle = {
        ...
        "trigger": "Run real HELL Linear project scenario through Performer/Conductor evidence harness",
...
def _flow_specific_evidence(
    ...
    common = {
        "workspace_evidence": {
            ...
            "validation_artifact": "PERFORMER_REAL_HELL_VALIDATION.md",
        },
        "tracker_evidence": {
    ...
    transitions_by_flow = {
        "FLOW-001": ["real_issue_created", "performer_instance_started", "codex_turn_completed", "linear_done"],
