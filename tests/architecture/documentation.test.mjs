...
test("Executor Markdown remains display-only and never pre-judges a fresh read-only Audit", async () => {
  ...
  assert.match(workflow, /Executor Markdown is copied byte-for-byte to the Execute Issue comment only/);
  assert.match(workflow, /does not repeat Cycle\s+description, acceptance, or boundaries/);
  assert.match(performer, /Execute Markdown is captured only for exact comment projection and is\s+untrusted; it is not parsed or supplied to Audit/);
...
test("Linear statuses are canonical, visible, and explicitly projected", async () => {
  ...
  assert.match(conductor, /\| Execute \| start a fresh workspace-write process; prompt requires final `cycle-NNN-executor-result\.md`/);
  assert.match(conductor, /\| Audit \| start a fresh read-only process; prompt requires final `cycle-NNN-audit-result\.md`/);
...
test("Audit Markdown is the sole semantic result and Root State is the Reconcile boundary", async () => {
  ...
  assert.match(workflow, /Audit Markdown is copied byte-for-byte to the Audit Issue comment only/);
  ...
  assert.match(rootIssue, /response is copied byte-for-byte to that role's Linear Issue comment only/);
  ...
  assert.match(reconciliation, /Root Reconcile\s+never receives the\s+complete Root Issue tree, Cycle Result, Cycle DAG/);
...
test("fresh Root Reconcile and visible Issue titles have closed contracts", async () => {
  ...
  assert.match(taskManagement, /`\[Cycle NNN\] <objective>` \(maximum 80 characters total\),\s+`\[Executor\] Cycle NNN`, and `\[Audit\] Cycle NNN`/);
});
...
test("diagnostic evidence stays private and non-semantic", async () => {
  ...
  assert.match(performer, /never supplied to the Audit prompt, Root Reconcile, or Linear comments/);
