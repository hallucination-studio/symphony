...
from conductor.linear import LinearProxyError, ManagedRunLinearProxy
...
async def test_linear_proxy_creates_a_parented_child_without_unused_label_lookup() -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        assert "parent { id identifier }" in query
                ...
                "data": {
                    "issueCreate": {
                        ...
                        "issue": {
                            ...
                            "title": "Implement",
                            "description": "Do it",
                            "parent": {"id": "parent-1", "identifier": "APP-1"},
                        },
                    }
                }
            },
...
@pytest.mark.anyio
async def test_linear_proxy_rejects_child_returned_under_another_parent() -> None:
    def handler(request: httpx.Request) -> httpx.Response:
        payload = json.loads(request.content)
        query = str(payload["query"])
        if "ManagedRunCreationContext" in query:
        ...
        return httpx.Response(
            ...
            json={
                "data": {
                    "issueCreate": {
                        "success": True,
                        "issue": {"id": "child-1", "identifier": "APP-2", "parent": {"id": "other-parent", "identifier": "APP-9"}},
                    }
                }
            },
        )
    ...
    with pytest.raises(LinearProxyError) as error:
        await proxy.create_child_issue_for(parent_issue_id="parent-1", title="Implement", description="Do it")

    assert error.value.code == "linear_child_parent_mismatch"
