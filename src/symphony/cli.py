from __future__ import annotations

import argparse
import asyncio
import logging
from pathlib import Path

from .config import ServiceConfig, load_env_file
from .http_server import SymphonyHttpServer
from .linear import LinearTracker
from .orchestrator import Orchestrator
from .persistence import PersistenceStore
from .reloader import WorkflowReloader
from .runner import AgentRunner
from .tracker import create_tracker
from .workflow import load_workflow
from .workspace import WorkspaceManager


def default_workflow_path(cwd: Path | None = None) -> Path:
    return (cwd or Path.cwd()) / "WORKFLOW.md"


def build_config_from_path(path: Path) -> ServiceConfig:
    load_env_file(path.parent / ".env")
    workflow = load_workflow(path)
    return ServiceConfig.from_workflow(workflow, path)


def apply_runtime_config(
    config: ServiceConfig,
    *,
    tracker: LinearTracker,
    runner: AgentRunner,
    orchestrator: Orchestrator,
) -> None:
    if hasattr(tracker, "update_config"):
        tracker.update_config(config.tracker)
    workspace_manager = WorkspaceManager(config.workspace, config.hooks)
    orchestrator.config = config
    orchestrator.workspace_manager = workspace_manager
    runner.config = config
    runner.workspace_manager = workspace_manager
    runner.codex_client.config = config.codex


def effective_server_port(config: ServiceConfig, cli_port: int | None) -> int | None:
    return cli_port if cli_port is not None else config.server.port


def persistence_store_from_config(config: ServiceConfig) -> PersistenceStore | None:
    if config.persistence.path is None:
        return None
    return PersistenceStore(config.persistence.path)


async def run_daemon(config: ServiceConfig, *, once: bool = False, port: int | None = None) -> None:
    logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
    config.validate_for_dispatch()
    tracker = create_tracker(config.tracker)
    workspace_manager = WorkspaceManager(config.workspace, config.hooks)
    runner = AgentRunner(config, workspace_manager, tracker=tracker)
    orchestrator = Orchestrator(
        config,
        tracker,
        runner,
        workspace_manager=workspace_manager,
        persistence_store=persistence_store_from_config(config),
    )
    orchestrator.load_persisted_state()
    http_server = await _maybe_start_http_server(config, orchestrator, port)
    await orchestrator.startup_terminal_workspace_cleanup(workspace_manager)
    try:
        while True:
            try:
                await orchestrator.tick()
            except Exception as exc:
                logging.exception("symphony_tick failed reason=%s", exc)
            if once:
                await orchestrator.wait_for_idle()
                return
            await asyncio.sleep(config.polling.interval_ms / 1000)
    finally:
        if http_server is not None:
            await http_server.stop()


async def run_reloading_daemon(workflow_path: Path, *, once: bool = False, port: int | None = None) -> None:
    logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
    reloader = WorkflowReloader(workflow_path)
    config = reloader.current()
    tracker = create_tracker(config.tracker)
    workspace_manager = WorkspaceManager(config.workspace, config.hooks)
    runner = AgentRunner(config, workspace_manager, tracker=tracker)
    orchestrator = Orchestrator(
        config,
        tracker,
        runner,
        workspace_manager=workspace_manager,
        persistence_store=persistence_store_from_config(config),
    )
    orchestrator.load_persisted_state()
    http_server = await _maybe_start_http_server(config, orchestrator, port)
    await orchestrator.startup_terminal_workspace_cleanup(workspace_manager)
    try:
        while True:
            config = reloader.current()
            apply_runtime_config(config, tracker=tracker, runner=runner, orchestrator=orchestrator)
            try:
                await orchestrator.tick()
            except Exception as exc:
                logging.exception("symphony_tick failed reason=%s", exc)
            if once:
                await orchestrator.wait_for_idle()
                return
            await asyncio.sleep(config.polling.interval_ms / 1000)
    finally:
        if http_server is not None:
            await http_server.stop()


async def _maybe_start_http_server(
    config: ServiceConfig, orchestrator: Orchestrator, port: int | None
) -> SymphonyHttpServer | None:
    if not config.observability.enabled:
        return None
    selected_port = effective_server_port(config, port)
    if selected_port is None:
        return None

    async def refresh() -> None:
        await orchestrator.tick()

    server = SymphonyHttpServer(config, orchestrator.state, refresh)
    await server.start(port=selected_port)
    logging.info("symphony_http outcome=started host=%s port=%s", config.server.host, server.port)
    return server


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the Symphony Linear/Codex daemon.")
    parser.add_argument("workflow", nargs="?", help="Path to WORKFLOW.md")
    parser.add_argument("--once", action="store_true", help="Run one poll cycle and exit after workers finish.")
    parser.add_argument("--port", type=int, help="Enable the HTTP status server on this port; 0 selects an ephemeral port.")
    return parser.parse_args(argv)


def main(argv: list[str] | None = None) -> int:
    args = parse_args(argv)
    path = Path(args.workflow).resolve() if args.workflow else default_workflow_path().resolve()
    try:
        asyncio.run(run_reloading_daemon(path, once=args.once, port=args.port))
    except KeyboardInterrupt:
        return 0
    except Exception as exc:
        print(f"symphony startup failed: {exc}", flush=True)
        return 1
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
