...
class RuntimeService:
    ...
    Enrollment tokens are single-use secrets tracked per workspace.
    """

    def __init__(self, store: PodiumStore):
        self.store = store
        # workspace_id -> enrollment token (pending enrollment)
        self._enrollment_tokens: dict[str, str] = {}
        # run_id -> RunSummary
    ...
    def generate_enrollment_token(self, workspace_id: str) -> str:
        """Generate a new enrollment token for a workspace runtime install."""
        token = secrets.token_urlsafe(32)
        self._enrollment_tokens[workspace_id] = token
        return token
    ...
    def enrollment_status(self, workspace_id: str) -> dict[str, Any]:
        ...
        return {
            ...
            "token_pending": workspace_id in self._enrollment_tokens,
    ...
    def record_heartbeat(self, runtime_id: str) -> None:
        """Record a heartbeat for a runtime, marking it online."""
