...
export function parseSymphonyRecordBlock(source) {
  if (typeof source !== "string") return failure("managed_record_block_invalid");
  const blocks = [...source.matchAll(SYMPHONY_BLOCK)];
  if (blocks.length === 0) return failure("managed_record_block_missing");
