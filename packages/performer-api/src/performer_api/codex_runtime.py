...
class RuntimePolicy:
    """Closed Symphony-owned policy for one managed Codex runtime."""
    ...
    def from_dict(cls, payload: dict[str, Any]) -> "RuntimePolicy":
        ...
        _reject_codex_owned_fields(payload)
    ...
    def create(
        ...
        *,
        ...
        return cls(
            ...
            execution_policy_sha256=_sha256(_canonical_json(normalized_execution_policy)),
            turn_policy=normalized_turn_policy,
            turn_policy_sha256=_sha256(_canonical_json(normalized_turn_policy)),
        )
    ...
    *,
    ...
    if isinstance(value, dict):
        for nested_key, nested_value in value.items():
            ...
            if _is_secret_key(nested_key) or nested_key in _CODEX_OWNED_FIELDS:
                raise RuntimePolicyError(
                    "runtime_policy_field_rejected",
                    "turn_policy cannot contain Codex-owned or secret fields",
                )
...
def _reject_codex_owned_fields(payload: dict[str, Any]) -> None:
    rejected = sorted(_CODEX_OWNED_FIELDS.intersection(payload))
    if rejected:
        raise RuntimePolicyError(
            "runtime_policy_field_rejected",
            f"Codex-owned field is not supported: {rejected[0]}",
        )
