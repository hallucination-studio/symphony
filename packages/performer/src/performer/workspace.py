...
class WorkspaceManager:
    ...
    async def _run_hook(self, name: str, script: str, cwd: Path, *, fatal: bool) -> None:
        ...
        try:
            ...
            logger.warning("performer_hook outcome=timeout hook=%s timeout_ms=%s", name, self.hooks.timeout_ms)
            if fatal:
                raise WorkspaceError("hook_timeout", f"{name} timed out") from exc
            return
        except OSError as exc:
            logger.warning("performer_hook outcome=failed hook=%s reason=%s", name, exc)
            if fatal:
        ...
        if process.returncode != 0:
            ...
            logger.warning(
                "performer_hook outcome=failed hook=%s exit_code=%s message=%s",
                name,
                process.returncode,
                message,
            )
            ...
            return
        logger.info("performer_hook outcome=completed hook=%s", name)
