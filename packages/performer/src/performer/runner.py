...
class AgentRunner:
    ...
    async def run_issue(
        ...
        try:
            ...
            logger.info(
                "performer_runner outcome=starting issue_id=%s issue_identifier=%s workspace=%s worker_host=%s",
                issue.id,
                issue.identifier,
                workspace.path,
    ...
    async def _continuation_prompt(self, issue: Issue, turn_count: int) -> str | None:
        ...
        if self.config.acceptance.enabled:
            return (
                ...
                "Performer will perform the In Review and Done state transitions after verification and acceptance."
            )
        return (
            f"Continue working on {issue.identifier}. This is turn {next_turn} of {self.config.agent.max_turns}. "
...
def _acceptance_agent_boundary_prompt() -> str:
    return "\n".join(
        [
            ...
            "- Do not move the Linear issue to In Review or Done.",
            "- Performer is the only actor allowed to perform Linear state transitions for review and completion.",
