...
- Linear comments used by tests must be explicit about intent. Human-action resumes happen through `[Human Action]` child issues; negative probes or diagnostic comments must identify themselves as such and must not look like business instructions.
...
- `docs/product/three-mode-runtime-pipeline.md` is the runtime architecture source of truth. Do not add direct polling, legacy phase scheduling, or `WORKFLOW.md` execution instructions.
- Runtime approval, permission, and tool-input waits must be projected to Linear as `[Human Action]` child issues and as node metadata (`operator_status: waiting_for_runtime_input`, `operator_wait_kind`, and a Runtime Wait block). Local stdout/logs alone are not an acceptable operator signal.
