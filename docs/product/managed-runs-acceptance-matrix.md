...
| A run-level block can retry only after a parent state flip; a plan revision remains blocked until an approved revised plan is recorded, with a durable and logged rejection reason. | `linear-projection.md`; `pipeline-state.md` | `test_managed_run_generic_parent_block_retries_only_after_root_state_flip`; `test_plan_revision_state_flip_stays_blocked_and_logs_required_resolution` | covered |
