...
that evidence to one read-only selected-backend Performer Gate turn.

## Pass rule

```text
passed = all verification commands passed
         and Performer Gate returned passed=true
         and score >= threshold
```

The backend result keeps the retained score, rubric rows, threshold, weights,
findings, acceptance-catalog id, manifest references, and artifact references.
Conductor binds the provenance to its actual Performer Gate attempt. These are
...
backend result, rubric, provenance, manifest, and artifact references. Linear
