...
- 默认创建独立 Linear Project；显式指定 Project slug 时使用并保留现有 Project，
  两种模式都只由runner创建本 run Root，随后由真实Root Agent创建Plan Work/Human children、
  Root Gate child、app data、`CODEX_HOME`、Git repository和evidence directory；
...
- fixture不得创建Root descendants；真实Plan必须通过Performer broker创建Work/Human children。
- Root有未解决blocker时必须保持childless、没有Primary Status Comment、没有Performer Turn，
  且不得进入Plan。
...
6. 创建带唯一 marker 和 Conductor Project Label 的 Linear Project/Root，并
   初始化干净的本地 Git repository；fixture在此处不创建任何Root descendants；
7. 验证有未解决blocker的Root没有claim、Primary Comment、Performer Turn或descendant child；
8. 观察eligible Root的真实Plan Turn通过broker创建Work child和Plan Approval Node，批准后执行Work Turn；
9. 观察Root Agent创建唯一`[Root Gate]` child，写入五项固定Markdown checklist，完成checks后将全部
   项更新为`[x]`并read-back；
10. 观察不同 Performer 进程使用同一 opaque `performer_id` resume conversation，并在Gate checked
    后完成本地 branch delivery、Root In Review和`in-review` phase；
...
| 10 | covered | 验证唯一`[Root Gate]` child、五项固定checklist的checked read-back和成功Root Gate；失败后Rework deferred |
...
`LINEAR_CLIENT_SECRET` 不属于 core live 输入，runner 不得读取或传递它。runner
不得主动加载 `.env` 或静态 Performer 文件；本机 `make e2e` 只允许在最后的 live
进程启动时通过 Node 原生 dotenv parser 加载仓库根 `.env`。base URL 允许 HTTP
或 HTTPS，不得包含
userinfo、query 或 fragment；CI 中 host 必须在 workflow 配置的 allowlist 中。
runner 必须为每类子进程构造显式环境 allowlist，默认不继承两个 token。

本机 credentialed 入口是 `npm run e2e:core-live`，它只从当前 shell 读取上述五项
输入。`make e2e` 负责安装、按 Podium → Conductor 顺序构建、运行 secret-free
runner contracts，再启动同一个 core-live entry；最后一步自动加载仓库根 `.env`，
且已有 shell 环境变量优先。Make 前置步骤显式移除两项 secret，只有最终 live
命令可读取它们。
