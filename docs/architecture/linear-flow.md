...
routing和bounded ownership record，不包含完整Cycle descendants或任何current Cycle/ready node副本。
...
-> mechanically reject terminal, waiting-human, ownership-conflict or incomplete candidates
-> choose first eligible Root by the fixed header order
-> fresh read selected complete Tree + Git again
-> finish an incomplete accepted directive, or call Root Reconciler for one directive
