...
Root Primary Status Comment保存full `conductor_id`。Conductor只恢复自己claim的Root；
...
- Root Primary Status Comment和Root Phase Labels；Timeline Comments不进入Tree工作流事实。
...
```text
Root -> In Progress
create Root Primary Status Comment
...
- 修改已有业务对象时验证`expected_updated_at`、预期state/parent和Managed Marker；
...
- Conductor只更新Symphony-origin Workflow Nodes、Root Phase Label、Root Primary Status
  Comment、Root Timeline Comments和Work Managed Metadata；
- Root Primary Status Comment中的Profile ID和usage字段只由Conductor更新；
- observation projection按`comment_id` upsert Primary或按`turn_id:sequence` append
  Timeline，不携带Root/comment revision，也不触发Root snapshot刷新；
