...
| Project Conductor Pool / Resolved Conductor Project | Linear Project上的Conductor Project Labels |
| Root routing | Root Issue上的唯一Root Conductor Label |
...
## 2. Project Conductor Pool与Root routing

Podium完成OAuth、Project catalog和Binding创建；Linear Project上的Labels声明允许并行执行该Project的
Project Conductor Pool：

```text
symphony:conductor/<short-hash>
```

每个Conductor仍以自己的Label解析Project。Conductor每个poll/Stage边界重新执行
`ResolveConductorProjectQuery`：

- 唯一Project携带自己的Label：返回Resolved Project和完整Project Conductor Pool；
- 无匹配：`unbound`；
- 多Project匹配：`conductor_project_ambiguous`；
- 同一Project上的其他合法Conductor Labels不是冲突。

顶层Root Issue可以携带同一namespace的唯一Root Conductor Label。它必须属于Project Conductor Pool，
并选择该Root唯一eligible Conductor：

```text
Project pool size == 1 and Root has no Conductor Label
  -> route to the sole member
Project pool size >= 1 and Root has exactly one in-pool Conductor Label
  -> route to that member
otherwise
  -> unrouted or conflicting; no Conductor may claim or dispatch
```

Root descendants不参与routing；在Cycle/Node上出现Conductor Label是malformed workflow fact。Project Label
是membership，Root Label是routing，Root Primary Status Comment中的full `conductor_id`是claim后的durable
ownership，三者不能互相替代。

Project pool和Root routing mutation携带expected membership、expected Root labels、expected ownership和
remote precondition。加入第二个pool member前必须先为所有非终态Root建立唯一in-pool routing；移除member
前必须证明没有非终态Root route或ownership仍引用它。Label变化不迁移Stage、branch或worktree。active
Root被改派时新旧Conductor都fail closed，直到未来显式quiesced transfer能力完成；本阶段不支持热迁移。

显式 quiescence 只能由operator针对已确认不再运行的E2E Root发起，将Root置为Canceled并等待
Linear read-back；它不是Conductor takeover、pool routing或ownership transfer。默认调度和并行
preparation不得自行执行该操作，活动Root仍必须fail closed。
...
```text
fresh validate Project membership + Root routing
Root -> In Progress
...
```text
  resolve Project and Project Conductor Pool
-> retain only Roots uniquely routed to this Conductor
...
- 验证current Conductor仍是Project pool member且Root唯一routing仍匹配它；
...
```text
Podium login and configure Project Conductor Pool
-> create or route each Root to one pool member
-> each Conductor resolves the shared Project and its own eligible Roots
-> full-scan routed Root headers
...
8. Project pool membership或Root routing变化不迁移ownership或Stage runtime；不匹配时相关Root fail closed。
...
15. 一个Project可以有多个不同Binding并行执行，但每个Root routing和durable ownership只允许一个writer；
    deterministic key不宣称替代跨writer CAS。
...
19. Cycle graph分阶段物化：先创建Bootstrap Plan，引用approved `plan_contract_digest`的exact graph在
    `Sealed`后才可调度。
20. Project pool多于一个member时，未标记Root不可调度；多个、pool外或descendant Conductor Labels都fail closed。
