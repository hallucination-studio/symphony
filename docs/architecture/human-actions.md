...
事实源。Cycle级Action内容由[Cycle Supervisor](cycle-supervisor.md)提出；Root等待与materialization由
[Root Reconciliation Loop](root-reconciliation.md)执行。
...
- archived Action仍属于完整Root/Cycle Tree并进入Supervisor observation。
...
但使Action退出active Tree membership；restore后仍需Supervisor决定是否创建新Action，不能隐式重放旧结果。
...
不能附加条件或悄悄改变scope。有条件同意应使用`Rejected`并写清希望如何修改，由Supervisor基于reason
...
-> pass missing-reason fact to Cycle Supervisor
-> Supervisor normally requests a linked Clarification Action
...
`Canceled`表示本次请求已失效或用户无法完成，不等于`Rejected`。Cycle级Action的后续由Supervisor决定：
...
或要求用户输入机器标识。内容超过bound时，Supervisor必须缩小请求或返回attention；Conductor不能创建内容
不完整的审批。
...
cycle_issue_id?
related_issue_ids[]
source_supervisor_directive_id?
...
## 8. Materialization与Supervisor恢复

### 8.1 创建Cycle Action

```text
Supervisor returns request_human_action directive
...
Cycle Action只能来自matching current Cycle Supervisor directive。Plan、Work和Verify只能返回typed需要事实，
不能直接创建Action或提供未经Supervisor选择的用户交互。

### 8.2 处理结果

```text
user changes Action status/comments
-> webhook wakes Root Loop
-> fresh read complete Action and related Cycle Tree, including archived Issues
-> validate and persist HumanActionResolutionRecord
-> read back resolution
-> rebuild complete Cycle Tree containing request, comments and resolution
-> advance the same Cycle Supervisor thread, or a fresh one after recovery
-> Supervisor chooses the next directive
```

Conductor不硬编码“Approved后执行Work”或“Rejected后replan”。具体下一步必须由Supervisor基于完整Tree决定，
但Conductor仍机械执行Plan Contract、permission、budget和status合法性约束。

## 9. Plan review durable facts

Plan Contract immutable。Plan Thread每次completed turn产生新的Plan Result和Contract digest；Supervisor决定是否
请求review。

```text
Plan Result durable
-> Supervisor requests Plan Review Action
-> Conductor creates Cycle child Action linked to Plan

Approved resolution durable
-> complete Tree to Supervisor
-> Supervisor may materialize proposed DAG and continue Work

Rejected resolution with reason durable
-> complete Tree to Supervisor
-> Supervisor may supersede old Contract and request a fresh Plan turn
...
移出active Tree，但每次Supervisor observation仍必须包含它们。
...
Permission resolution进入完整Tree后由Supervisor选择下一步。Conductor只在matching turn request中授予精确
...
- 用户修改Root/Cycle/DAG或Action期间产生的旧Supervisor directive在digest检查时被拒绝；
...
5. Cycle Action内容和后续语义由Cycle Supervisor决定；Conductor拥有materialization和校验。
