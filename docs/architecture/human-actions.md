...
```symphony
{"kind":"human_action_issue","version":1,"record_id":"...",...}
```
````

renderer按Action kind省略不适用状态，不显示空section。description中的唯一`symphony` block只承载Issue identity、kind、
parent scope和source directive reference；完整immutable proposal仍写入`HumanActionRequestRecord` comment，避免description
编辑成为第二份resolution或approval事实。
