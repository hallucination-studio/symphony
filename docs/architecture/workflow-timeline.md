...
Stage Result自身的immutable Result、model和turn usage写在matching Plan/Work/Verify Issue managed comment；Cycle timeline
event只引用该source record并展示本次结果与派生Cycle累计值。前者是canonical Result materialization，后者是typed
timeline event；二者使用不同record kind，不形成第三条Workflow timeline，也不得把Cycle累计snapshot再次计入usage。
...
`ModelTurnRecord`计数。Timeline用于用户理解和审计，不是运行日志镜像。
