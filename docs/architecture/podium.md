...
Conductor Label；移除member前，不得存在route或Root Primary ownership仍指向它的非终态Root。
任一条件不满足都fail closed，不做partial pool mutation。

Podium Desktop/Conductor runtime必须遍历全部Binding。Overview按Conductor显示Project pool，
按Root显示唯一routing和Primary ownership关系；同一Project上的多个合法Conductor不是
Project conflict，unbound、ambiguous、routing conflict和ownership conflict必须分别显示。
...
label。Podium不写Root Primary ownership、Cycle、Node或Workflow evidence。Project只有一个pool member时，
