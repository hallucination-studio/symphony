...
3. Root Loop保持确定性；Cycle语义只由Supervisor模型决定。
...
- 定义Supervisor observation/directive schemas；
...
- Root Loop中不存在model或Agent SDK。

## 4. R2：Cycle Supervisor

- Performer创建每Cycle独立Supervisor thread；
- Conductor发送完整active+archived Cycle observation；
- Supervisor返回一个closed `CycleDirective`；
- Conductor校验Tree digest、persist directive、materialize/read-back；
- 支持create/update/archive/restore/reorder/dependency patch；
- thread丢失后使用完整Tree打开fresh Supervisor session。

## 5. R3：Plan、Work、Verify role threads

- 每Cycle创建与Supervisor隔离的Plan、Work、Verify threads；
- Plan/Verify read-only，Work workspace-write；
- Work thread跨多个Work Issues和turn复用；
- Work turn内部可以诊断普通错误、修改和重试；
- Result先durable，再进入Supervisor observation；
...
- Supervisor生成完整Action proposal，Conductor materialize；
- 用户status/comment形成closed resolution并返回Supervisor；
- Rejected missing reason和Answered missing answer fail closed并进入Supervisor处理；
...
2. Conductor通过真实session protocol驱动Supervisor、Plan、Work、Verify四个隔离threads。
3. Work thread跨至少两个Work Issues连续执行，普通错误在turn内恢复。
4. Supervisor依据Result调整DAG、创建Human Action并在resolution后继续。
