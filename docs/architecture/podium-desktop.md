...
必须明确说明：关闭Desktop会暂停本地Conductor；下次打开后从Linear和Git恢复，并为下一次执行创建
fresh Stage context。
...
- Root问题和Pending Human Action以当前Root摘要进入Overview；完整Tree仍在Linear；
...
- Project；
- 用户可见Root状态；
- 当前Cycle authoritative status、Plan/Work/Verify Node或Pending Human Action；
...
- Pending Human Action或blocked reason；
- 当前Root固定的Performer Profile；
...
`Activate`不重启Conductor，立即改变新Root使用的Profile。若当前Stage或已有Root使用
旧Profile，页面同时显示：
...
编辑同一个Profile的model、reasoning、Fast或execution policy后，该Profile下一个Stage使用新设置；当前
Stage继续。Desktop必须区分保存成功与Conductor拒绝：Update失败时保留并显示原设置，不能
乐观显示尚未被Conductor接受的新值。

若用户在Stage运行时发起Login或Status refresh，Desktop显示`Waiting for current
stage`；Edit和Activate不需要等待。
...
Conductor
  ...
  -> one short-lived Performer process per Stage
```

Desktop不直接调用Performer。Conductor为已选择的Plan、单一Work target或Verify构造StageContext并建立
StageWire；每个短进程都使用fresh Provider context。
...
- Root、Tree、Work、Verify或Pending Human Action状态；
- Root Activity投影；
- StageWire、Stage Result或Provider thread identity；
...
5. Performer由Conductor通过caller-owned StageWire调用。
