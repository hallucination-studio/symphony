...
- 同一个事实只显示一个主要状态，不同时堆叠Root state、Root Phase Label和Turn state；
...
必须明确说明：关闭Desktop会暂停本地Conductor；下次打开后从Linear、Git和
`performer_id`恢复。
...
- Root Gate是Root Phase，不是用户单独管理的对象；
...
6. 保存Profile；
7. 完成ChatGPT Login或输入API Key；
8. Profile变为Ready后设为active。
...
当前Turn可以用Usage Event临时刷新数字；Turn结束后以Result累计值替换。Event丢失或
应用重启时直接回到Conductor从Linear读取的累计值，不显示虚构的连续实时曲线。
...
7. **Delivery**：只有进入delivering/in-review后显示安全摘要；
8. **Problem details**：error code对应的用户说明、影响和下一动作；
9. **Advanced details**：默认折叠，只显示安全ID、phase和observed time。
...
编辑同一个Profile的model、reasoning或Fast后，该Profile下一个Turn使用新设置；当前
...
Linear terminal state优先于phase：

| Linear/phase事实 | UI label |
|---|---|
| Root Done | `Completed` |
...
| `working` | `Working` |
| `gating` | `Reviewing result` |
| `delivering` | `Preparing delivery` |
| Root In Review / `in-review` | `Ready for review` |
| `blocked` | `Action required` |
| `failed` | `Failed` |

状态旁必须显示文字和图标；不能只显示颜色。内部Root Phase Label可以放在Advanced details，
...
| 执行Plan、Work、Gate、commit和delivery | Conductor/Performer，用户无强制按钮 |
...
Root的`RootAction`和等待原因来自Conductor的当前报告；Podium只做安全字段
...
Desktop不常驻启动Performer。Performer由Conductor在需要Plan、Work或Root Gate时启动，
Turn结束后退出。
