...
process exit、heartbeat或日志翻译成Workflow状态。
...
handshake或heartbeat只能更新当前内存presence。它们不是lease、workflow cursor、Root ownership或恢复事实。Podium
...
unexpected exit或heartbeat丢失使当前presence变为offline并产生脱敏日志。Desktop不根据该事件修改任何Linear对象，
