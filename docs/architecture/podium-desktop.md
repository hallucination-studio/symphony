...
- Project；
- 用户可见Root状态；
- 当前Cycle authoritative status、Supervisor decision、Plan/Work/Verify Node或Human Action；
...
Desktop不直接调用Performer。Conductor主动驱动每Cycle隔离的Supervisor、Plan、Work和Verify sessions；
Supervisor/Stage contracts不暴露给Desktop。
...
- Root、Tree、Supervisor、Work、Verify或Human Action状态；
- Root Activity投影；
