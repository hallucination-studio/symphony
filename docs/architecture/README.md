...
## 3. Stage orchestration与Human Action

Conductor Loop、Plan/Work/Verify的公共注入Envelope、三个context variants、tool capability和Result只由
[Stage Context](stage-orchestration.md)定义。用户审批、拒绝、信息补充、权限和override的交互、状态及恢复
只由[Human Action](human-actions.md)定义。本文不重复这些设计。
...
- [Linear Workflow Loop与Performer Stage Context](stage-orchestration.md)：Loop、注入context、capability和Result。
- [Human Action交互与恢复](human-actions.md)：用户审批、拒绝/replan、信息、权限、状态和durable resolution。
- [Root Issue工作流](root-issue.md)：Linear Team status、分阶段Cycle graph、Finding和Delivery事实。
