...
| Control/Timeline/Reply comments | ownership/Profile事实或用户叙事与回复 | Workflow lifecycle、调度cursor |
...
- Node terminal status必须有matching terminal Result；只有Result而status未terminal时只补齐同一execution materialization；
- Human Action terminal status只有在matching `HumanActionResolutionRecord`成立后才产生workflow后果；缺reason/answer时
  保留用户选择的Linear status但Root停止，等待用户补充，不能伪造resolution或回滚status；
- Cycle terminal status必须有matching `CycleOutcome`且映射唯一；不一致时fail closed；
- `RootDirectiveRecord`只证明directive已接受，不证明其mutation、reply或timeline已经完成；
- Timeline/Reply marker只证明matching Linear comment存在，不证明Workflow进入新状态。

## 8. Result与Finding

Plan/Work/Verify Result先按role/session/turn/context/Git preconditions验证，再写matching Node managed comment并
read-back。Result不能直接改变下一步；完整Root Tree交给Root Reconciler后，由directive决定。
...
`superseded`表示Root contract实质变化或当前Cycle无法安全吸收破坏性revision；它不是execution failure，也不会
...
activity、usage、branch/delivery副本、Queue或Provider pointer。branch和delivery从Git读取，Workflow lifecycle从Linear
...
任何合法Linear变化都进入fresh Tree和Root Reconciler observation：

- Root目标、scope或acceptance变化使旧Tree digest和相关directive失效；
- 用户修改pending Work内容后，Root Reconciler重新评估；
- 用户archive/restore managed Node后，Conductor验证DAG并让Root Reconciler决定恢复；
- Human Action status/comment形成validated resolution后交给Root Reconciler；
- human actor创建且不含managed marker的普通comment进入pending input，并在处理后收到matching reply；
- 用户伪造marker、跨Root relation、非法status或造成active dependency悬空时matching Root fail closed并写Linear timeline；
...
7. Stage Result必须durable后才能进入Root Reconciler observation。
8. Cycle耗尽先走Root convergence gate，不机械请求用户。
9. Timeline comment是durable事实的用户叙事；Root Reconciler reply是directive的必需mutation。两者都不是workflow输入。
