...
`pending_user_comments`不是持久集合。Conductor每次从完整Linear comments减去matching accepted directive disposition和
`RootReconcilerReplyRecord`后派生；只包含human actor创建且没有Symphony managed marker的普通comment。必须排除：
...
Linear observed status本身是事实，不存在内部expected-status镜像。该variant只在完整Tree形成非法组合时，提出一次
新的显式Linear status mutation及理由；例如Node被手工置为Done但没有matching Result。它不得撤销合法Human terminal
status、伪造Stage完成或绕过Root cancel。若用户状态变化表达明确合法意图，Reconciler必须选择matching
lifecycle/replan/supersede directive；无法判断时请求Human Action，不能静默改回。
...
再创建fresh Cycle和三个fresh Stage sessions。`invalidated_delivery_refs`引用Git中的旧PR、branch或commit，说明其
...
Root Reconciler负责语义分类，但以下结果不是自由裁量：Root `Canceled`由Conductor立即停止新dispatch并取消matching
execution，不等待也不创建`conclude_root` directive；没有matching Result
...
Root Reconciler只在durable Linear/Git边界调用：新用户comment、Stage Result、Human resolution、外部Tree变化、
...
closed coverage明确缺失并使matching Root fail closed，或使用由durable source支持的bounded history view；不能静默截断或让
旧transcript补全事实。
