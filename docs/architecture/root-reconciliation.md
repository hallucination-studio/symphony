...
其中用户comment事实包括human-authored body versions以及human actor执行的native thread changes，即使thread target是
Symphony timeline或reply comment。reaction current set只用于回执read-back和审计，不成为Root pending input。
...
RootDeltaChange =
  ...
  IssueDetached |
  CommentCurrentValue |
  CommentThreadChangeCurrentValue |
  CommentRemoved |
...
description变化发送新的完整description；comment新增或编辑发送新的完整body与native thread resolution；
...
resolve/unresolve传送新的完整comment business value，reaction-only变化明确排除；status、archive、parent和relation传送新的当前值，
...
comment body versions、human thread changes、Git/delivery事实和mechanical violations。Timeline/Reconciler reply等
automation comment body不属于该fact set，但human actor针对它们产生的thread change属于；Symphony自身的
reply/reaction/resolve write按stable correlation排除。Raw SDK对象、transport heartbeat和其他明确排除的
...
pending comment input不是持久集合。Conductor每次从完整Linear comments/thread changes减去accepted directive中的
`consumed_input_ids[]`后派生。comment body version和thread change是两个closed input variant；后者按change actor而不是
comment author分类，因此用户可以reopen Symphony timeline/reply thread而不被错误过滤。reaction不是pending input。
必须排除：
...
body input identity是`comment_id + comment_version`；thread input identity是Linear source change identity。每个identity
最多处理一次。只有human body/thread change进入pending input；matching Symphony reply/reaction/resolve writes按stable
...
每个被消费且target thread/comment仍存在的用户comment body或thread input必须由同一个
...
UserCommentReply
  source_input_id
  source:
    CommentVersionReplySource
      comment_id
      comment_version
    | CommentThreadChangeReplySource
      thread_change_id
      target_comment_id
  acknowledgement
  interpreted_request
...
RootReconcilerReplyRecord
  ...
  source_input_id
  source:
    CommentVersionRecordSource
      comment_id
      comment_version
    | CommentThreadChangeRecordSource
      thread_change_id
      target_comment_id
  target_issue_id
  disposition
...
中不存在的输入。
`comment_replies[]`必须精确覆盖其中全部target仍存在的用户comment body/thread inputs，并排除comment
tombstones。无业务影响时也
...
- 不存在Linear之外的pending reply/timeline状态。恢复只根据Linear source record与matching managed comment是否
  存在继续同一写入；timeline/reply automation body不会作为新的用户输入，但human在其thread中新增comment或执行
  reopen/resolve仍形成正常pending input。
