...
增量按来源携带当前值，不携带人为构造的旧值/新值对：description变化传送新的完整description，comment新增或编辑
传送新的完整body，status、archive、parent和relation传送新的当前值，删除或脱离Tree传送明确tombstone。source
...
并在Linear read-back后才算完成。写入失败就停止当前Root并记录错误，不创建另一份“待投影”或“待确认”状态。
