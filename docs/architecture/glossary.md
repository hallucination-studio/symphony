...
| Performer | `apps/performer` | 承载Cycle Supervisor和Plan/Work/Verify role threads的Python Agent runtime | Worker Service、Codex Runner |
...
| Root Action | `RootAction` | Root Loop的一次性机械动作，只存在于当前reconciliation内存 |
| Cycle Directive | `CycleDirective` | Cycle Supervisor基于完整Tree返回的一个closed语义下一步proposal |
...
| `cycle-supervisor-client` | 拥有`CycleSupervisorClientInterface` | 构造完整observation并调用Performer Supervisor |
| `cycle-directive-materialization` | 拥有`CycleDirectiveMaterializerInterface` | 验证、幂等执行和read-back directive |
| `performer-agent-client` | 拥有`PerformerAgentClientInterface` | 驱动四role session/turn request-response |
| `workflow-events` | 拥有`WorkflowTimelinePublisherInterface` | 发布typed Root/Cycle timeline event |
| `timeline-projections` | timeline subscribers | 渲染并投影Root/Cycle Linear comments |
...
|---|---|---|
| Cycle Supervisor Session | `CycleSupervisorSession` | 一个Cycle专属的模型ReAct role thread；只返回CycleDirective |
| Role Session | `RoleSession` | 一个Cycle内Supervisor、Plan、Work或Verify的隔离Provider thread runtime |
...
Plan、Work、Verify request/result语义只由[Stage Contracts](stage-orchestration.md)定义；Supervisor语义只由
[Cycle Supervisor](cycle-supervisor.md)定义。当前不设计role内部sub-agents或第二Provider。
...
| Turn Usage | `StageUsageSnapshot` | 一次Supervisor/Stage turn Result携带的Codex SDK token使用量 |
...
- `RootAction`使用closed mechanical variants，例如`create_initial_cycle`、`advance_cycle_supervisor`、
  `materialize_cycle_directive`、`create_successor_cycle`、`deliver_root`、`wait`和`mark_attention`。
- `CycleDirective`使用closed semantic variants：`execute_plan`、`revise_cycle_tree`、`execute_work`、
  `execute_verify`、`request_human_action`、`wait`和`conclude_cycle`。
