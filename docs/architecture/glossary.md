...
| Root Invariant Policy | `RootInvariantPolicyInterface` | 只验证ownership、status、budget、convergence和mutation preconditions，返回valid/invalid与机械原因 |
...
| Root Reconciler Reply | `RootReconcilerReplyRecord` | matching RootDirective处理用户comment后写回原Issue并read-back的必需受管回复 |
...
Root Control Record Comment，`event_key` variant append Root Timeline Comment。
...
| `root-reconciliation` | 拥有`RootInvariantPolicyInterface` | 从fresh facts验证机械不变量；不选择业务下一步 |
...
- `RootDirective`使用closed semantic variants：`execute_plan`、`execute_work`、`execute_verify`、`rerun_stage`、
  `resolve_invalid_lifecycle`、`revise_cycle_tree`、`replan_current_cycle`、`supersede_cycle`、
