...
test("description timestamps use a human-readable local time and numeric timezone", () => {
  ...
  assert.equal(timestamp, `${expected} GMT${offset}`);
...
test("description timestamp parser rejects UTC suffixes and impossible calendar values", () => {
  for (const value of [
    ...
    "2026-02-30 03:04:05 GMT+08:00",
    "2026-01-02 03:04:05 GMT+8:00",
  ]) {
