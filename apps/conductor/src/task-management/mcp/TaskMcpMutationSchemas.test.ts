...
test("mutation results expose exactly five closed outcomes with fresh facts", () => {
  ...
  for (const outcome of ["not_applied", "precondition_failed", "acceptance_unknown", "readback_mismatch"] as const) {
    assert.equal(parseTaskMcpResult({
      ...envelope("update_issue"),
      output: {
        outcome,
        target: { kind: "issue", issue_id: "LIN-2" },
        fresh_resource: outcome === "precondition_failed" ? issue : null,
        concrete_diff: [],
