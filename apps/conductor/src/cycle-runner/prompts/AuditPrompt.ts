...
export function renderAuditPrompt(
  ...
  const processFacts = renderRuntimeContext("EXECUTE_PROCESS_FACTS", JSON.stringify({
  ...
  return parseMarkdownText([
    "You are Symphony's Audit role. Independently audit the complete real workspace for exactly one frozen Cycle.",
    ...
    "- EXECUTE_PROCESS_FACTS are mechanical process facts only. A nonzero exit does not prove success or failure.",
    "- Executor prose is unavailable and must not be inferred. Inspect the workspace and run read-only checks yourself.",
    ...
    "- use accepted only when the complete real workspace satisfies the Cycle and its boundaries; use incomplete for remediable missing work; blocked for an external blocker; violation for integrity or boundary violations; process_error only when Audit itself cannot produce a semantic judgment.",
    ...
    "For process_error, return only `verdict: process_error` and `## Reason`, preserving the current Audit error message.",
