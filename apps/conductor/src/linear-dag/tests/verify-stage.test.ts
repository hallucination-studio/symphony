...
function verifyTree(): LinearWorkflowTreeSnapshot {
  const statuses = (["Draft", "Todo", "Planning", "Sealed", "Executing", "Verifying", "In Progress", "In Review", "Needs Approval", "Needs Info", "Inconclusive", "Escalated", "Succeeded", "Changes Required", "Done", "Canceled", "Failed", "Duplicate"] as const).map((name, position) => ({ status_id: `status-${name.toLowerCase().replaceAll(" ", "-")}`, name, category: (["Draft"] as string[]).includes(name) ? "backlog" as const : (["Todo"] as string[]).includes(name) ? "unstarted" as const : (["Succeeded", "Changes Required", "Done"] as string[]).includes(name) ? "completed" as const : (["Canceled", "Failed", "Duplicate"] as string[]).includes(name) ? "canceled" as const : "started" as const, position }));
