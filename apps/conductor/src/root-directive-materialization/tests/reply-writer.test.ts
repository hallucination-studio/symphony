...
test("reply writer materializes a native child reply and receipts the human source once", async () => {
  ...
  assert.deepEqual(result, { kind: "materialized", replyId: candidate.replyId });
  ...
  assert.doesNotMatch(created.body, /<!-- symphony/u);
  const parsed = parseManagedRecord(created.body);
  assert.equal(parsed.ok, true);
  if (!parsed.ok || parsed.value.kind !== "root_reconciler_reply") return;
  assert.deepEqual(parsed.value.source, candidate.source);
  assert.equal(parsed.value.replyId, candidate.replyId);
  ...
  assert.equal(receipt(linear.reply(candidate.replyId)), "none");
...
function view(tree: LinearWorkflowTreeSnapshot): RootReconciliationView {
  return {
    root: {
      ...
      updatedAt: "2026-07-23T00:00:00Z", projectId: "project-1", parentIssueId: null, isDelegatedToSymphony: true,
      priority: "normal", order: 0, blockers: [], rootConductorLabels: [{ conductorShortHash: "abc123" }],
    },
    tree,
...
class FakeLinear {
  ...
  constructor(input: { body?: string; remoteVersion?: string; threadState?: "resolved" | "unresolved"; omitCreatedReply?: boolean } = {}) {
    ...
    this.tree = {
      ...
      relations: [], source_manifest: [], coverage: { is_complete: true, omissions: [] }, observed_at: "2026-07-23T00:00:02Z",
    };
  ...
  reply(replyId: string) {
    const reply = this.tree.comments.find(({ parent_comment_id, body }) =>
      parent_comment_id === "comment-1" && body.includes(replyId));
...
function issue(issueId: string, identifier: string, parentIssueId: string | undefined, title: string, depth: number) {
  return {
    ...
    remote_version: `${issueId}-v1`, updated_at: "2026-07-23T00:00:00Z",
  };
}
